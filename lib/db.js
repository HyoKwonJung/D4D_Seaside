...
async function upsertEvent(event) {
  ...
  try {
    await client.query("BEGIN");
    // vessel_mmsi 는 vessels 테이블 FK. 다크 탐지처럼 실제 AIS 선박이 없는 이벤트
    // (mmsi 없음, vessel_id 는 dark-track ID)는 NULL 로 저장한다.
    let vesselMmsi = null;
    if (event.mmsi) {
      const vesselRow = await client.query("SELECT 1 FROM vessels WHERE mmsi = $1", [String(event.mmsi)]);
      vesselMmsi = vesselRow.rowCount ? String(event.mmsi) : null;
    }
    await client.query(`
      INSERT INTO events (
        id,
