...
function deriveEncounterEvents(vessel, liveVessels, trackHistory, cables, watchArea, vtsLocations) {
  ...
  liveVessels.forEach(candidate => {
    ...
    if (isAnchoredOrMoored(candidate)) return;
...
function deriveLiveEvents(options) {
  ...
  if (isVtsIdentified(vessel, vtsLocations)) return [];
  if (isAnchoredOrMoored(vessel)) return [];
...
// AIS 항해 상태 1=묘박(at anchor), 5=계류(moored).
// 입항대기 선박의 저속·장시간·근접은 통상 활동이므로 이상행동 후보에서 제외한다.
function isAnchoredOrMoored(vessel) {
  if (!vessel) return false;
  const status = Number(vessel.nav_status);
