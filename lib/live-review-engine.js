...
const { getContextNote } = require("./attack-strategy-reference.js");
...
function deriveCableRiskEvent(vessel, track, cables, watchArea, watchZone) {
  ...
  return domain.enrichEvent({
    ...
    evidence: [
      ...
      getContextNote("live_ais_review")
    ].filter(Boolean),
    scoring_version: "v1",
...
function deriveLoiteringEvent(vessel, track, cables, watchArea, watchZone) {
...
function deriveEncounterEvents(vessel, liveVessels, trackHistory, cables, watchArea, vtsLocations, watchZone) {
...
function deriveLiveEvents(options) {
  ...
  const watchZone = options && options.watchZone;
  const vtsLocations = options && options.vtsLocations;

  if (isVtsIdentified(vessel, vtsLocations)) return [];

  const results = [];
  const cableRiskEvent = deriveCableRiskEvent(vessel, track, cables, watchArea, watchZone);
  if (cableRiskEvent) results.push(cableRiskEvent);

  const loiteringEvent = deriveLoiteringEvent(vessel, track, cables, watchArea, watchZone);
  if (loiteringEvent) results.push(loiteringEvent);

  return results.concat(deriveEncounterEvents(vessel, liveVessels, trackHistory, cables, watchArea, vtsLocations, watchZone));
}
